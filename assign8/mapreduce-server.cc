...
// zgoz
void MapReduceServer::stageMappedFiles(const std::string& directory, list<string>& files) throw() {
  for (size_t i = 0; i < (numMappers*numReducers); i++) {
    string filesWildCard(directory);
    filesWildCard += "/*.";
    filesWildCard += numberToString(i);
    filesWildCard += ".mapped";
    files.push_back(filesWildCard);
  }
}
...
bool MapReduceServer::surfaceNextFilePattern(std::string& pattern) throw() {
  lock_guard<mutex> lg1(unprocessedMutex);
  if (unprocessed.empty()) return false;
  pattern = unprocessed.front();
  unprocessed.pop_front();
  lock_guard<mutex> lg2(inflightMutex);
...
void MapReduceServer::spawnMappers() throw() {
  ...
  for (size_t i = 0; i < numMappers; i++) {
    const string& mapperNode = mapperNodes[i % mapperNodes.size()];
    string command = buildMapperCommand(mapperNode, mapperExecutable, intermediatePath);
    threads.push_back(
      thread(&MapReduceServer::spawnWorker, this, mapperNode, command)
    );
  }
...
void MapReduceServer::spawnReducers() throw() {
  ...
  for (size_t i = 0; i < numReducers; i++) {
    const string& mapperNode = mapperNodes[i % mapperNodes.size()];
    string command = buildReducerCommand(mapperNode, reducerExecutable, outputPath);
    threads.push_back(
      thread(&MapReduceServer::spawnWorker, this, mapperNode, command)
    );
  }
  if (verbose) cout << "Reducing of all mapped chunks now complete." << endl;
...
string MapReduceServer::buildMapperCommand(const string& remoteHost, 
  ...
  size_t hashCount = numMappers * numReducers;
